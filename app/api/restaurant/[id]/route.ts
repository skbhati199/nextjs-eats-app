...
import { deleteByIdRestaurant, getByIdRestaurant } from "@/lib/restaurant-helpers";
...
export async function GET(
  ...
  try {
    const res = await getByIdRestaurant(params.id);
...
export async function DELETE(
  ...
  try {
    ...
    const res = await deleteByIdRestaurant(params.id);
    return NextResponse.json(res, { status: 200 });
  } catch (error) {
